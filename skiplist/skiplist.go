...
func (s *Skiplist) Iterator() *Iterator { return &Iterator{current: s.head.forward[0]} }
...
func (it *Iterator) Key() Key { if it.current == nil { return Key{} }; return it.current.Key }

func (it *Iterator) SeriesID() uint64 { if it.current == nil { return 0 }; return it.current.Key.SeriesID }
...
func (s *Skiplist) Search(key Key) (*node, bool) {
	...
	for level := s.currTopLevel - 1; level >= 0; level-- {
		for current.forward[level] != nil && s.comparator(current.forward[level].Key, key) < 0 {
